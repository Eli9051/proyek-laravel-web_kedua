...
<body class="bg-[#f2f2f7] font-sans antialiased text-[#1f1f1f]" x-data="{ sidebarOpen: true }">
    <div class="flex min-h-screen">
        <aside
            ...
            <nav class="flex-1 p-4 space-y-1 overflow-y-auto">
                ...
                <a href="https://wa.me/081240590094?text=Halo%20HR%20MyAbsensi,%20saya%20ingin%20bertanya%20terkait..."
                    target="_blank"
                    class="flex items-center gap-4 p-3 hover:bg-gray-800 rounded-xl transition-all font-medium group">
                    <i data-lucide="life-buoy" class="w-5 h-5 text-red-400 group-hover:animate-spin"></i>
                    <span x-show="sidebarOpen">Bantuan HR</span>
                </a>
                @if(Auth::user()->role === 'hr')
                <a href="#" class="flex items-center gap-4 p-3 hover:bg-gray-800 rounded-xl transition-all font-medium group">
                    <i data-lucide="shield-alert" class="w-5 h-5 text-red-500 group-hover:animate-bounce"></i>
                    <span x-show="sidebarOpen">Log Keamanan Absen</span>
                </a>
                @endif
            </nav>
            ...
            <div class="p-4 border-t border-gray-700 bg-[#191c24]">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button class="flex items-center gap-4 p-3 text-red-500 font-bold w-full hover:bg-gray-800 rounded-xl transition-all">
